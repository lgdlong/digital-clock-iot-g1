...
float convertAdcToTemperature(int adcValue) {
  ...
  if (adcValue == 0) return 25.0; // Default temperature
    ...
    for(int i = 0; i < alarmCount; i++) {
      ...
      for(int j = 0; j < 7; j++) {
        if(alarms[i].daysOfWeek[j]) {
          if(hasActiveDays) activeDays += ", ";
          activeDays += days[j];
          hasActiveDays = true;
        }
      }
      if(!hasActiveDays) activeDays += "Không lặp lại";
...
void loop() {
  ...
  if (timer.alarmTriggered) {
    ...
    if (alarmElapsed < 5000) { // 5 seconds alarm
      static unsigned long lastBlinkTimer = 0;
      if (millis() - lastBlinkTimer > 250) { // Fast blink for timer alarm
...
void fetchWeatherData() {
  ...
  unsigned long now = millis();
  if (now - weather.lastUpdate < weatherConfig.updateInterval * 1000) return;
