...
String generateWebInterface()
{
  ...
  html += "</form>";
  // html += "<form action='/status' method='GET' style='display:inline;'>";
  // html += "<button type='submit' class='btn'>🔄 Lam moi thoi tiet</button>";
  // html += "</form>";
  html += "</div>";
  ...
  html += "</div>";
  html += "</div>";
  html += "<button type='submit' class='btn btn-success'>➕ Them bao thuc</button>";
  html += "</form>";
  ...
  // html += "<div style='text-align: center;'>";
  // html += "<button onclick=\"restart()\" class='btn'>🔄 Khoi dong lai</button>";
  // html += "<button onclick=\"factoryReset()\" class='btn btn-danger'>⚠️ Khoi phuc goc</button>";
  // html += "</div>";
  html += "<div style='margin-top: 15px; font-size: 2rem; opacity: 0.8;'>";
  ...
  html += "<script>";
  html += "let lastServerTime=null;let lastUpdateMillis=null;";
  html += "function updateStatus(){fetch('/status').then(r=>r.json()).then(data=>{";
  html += "if(data.time){lastServerTime=new Date(data.time);lastUpdateMillis=Date.now();}";
  html += "const clockEl=document.querySelector('.clock-display');";
  html += "if(clockEl&&lastServerTime){const now=new Date(lastServerTime.getTime()+(Date.now()-lastUpdateMillis));const h=now.getHours().toString().padStart(2,'0');const m=now.getMinutes().toString().padStart(2,'0');const s=now.getSeconds().toString().padStart(2,'0');clockEl.innerHTML=h+':'+m+':'+s;}";
  ...
  // Alarm day auto-select logic
  html += "document.addEventListener('DOMContentLoaded',function(){";
  html += "const alarmForm=document.querySelector('form[action=\'/set-alarm\']');if(alarmForm){alarmForm.addEventListener('submit',function(e){const dayCheckboxes=[...alarmForm.querySelectorAll('input[type=\'checkbox\'][name^=day]')];if(dayCheckboxes.every(cb=>!cb.checked)){const now=new Date();const hour=parseInt(alarmForm.querySelector('input[name=\'hour\']').value)||0;const minute=parseInt(alarmForm.querySelector('input[name=\'minute\']').value)||0;let alarmDay=now.getDay();if(hour<now.getHours()||(hour==now.getHours()&&minute<=now.getMinutes())){dayCheckboxes[alarmDay].checked=true;}else{dayCheckboxes[(alarmDay+1)%7].checked=true;}}});}});";
...
void setupWebServer()
{
  ...
  server.on("/status", HTTP_GET, []()
    ...
    // Add current time in ISO format for web clock sync
    DateTime now = rtc.now();
    char isoTime[25];
    snprintf(isoTime, sizeof(isoTime), "%04d-%02d-%02dT%02d:%02d:%02dZ", now.year(), now.month(), now.day(), now.hour(), now.minute(), now.second());
    doc["time"] = isoTime;
